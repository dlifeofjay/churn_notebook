...
5. Demonstrate a simple **model deployment** using a decoupled architecture (FastAPI backend + Streamlit frontend).
...
├── flask_stream.py         # Streamlit frontend application (client)
├── main.py                 # Entry point to run the backend server
├── preprocessing.py        # Shared Pydantic models and preprocessing logic
├── ecom_churn.py           # Standalone Streamlit app (alternative)
├── col_encoder.pkl         # Saved ColumnTransformer
├── model_cus.pkl           # Trained churn prediction model
├── ecom cus.ipynb          # Model training and analysis notebook
...
2.  **Install dependencies:**
    ...
    ```bash
    pip install fastapi uvicorn streamlit pandas scikit-learn joblib requests
    ```
...
This project uses a decoupled architecture with a **FastAPI backend** for predictions and a **Streamlit frontend** for user interaction.
...
```bash
streamlit run flask_stream.py
```

- This launches the dashboard in your default web browser.
- You can now enter customer details and click **"Predict"** to get real-time churn probabilities from the API.
